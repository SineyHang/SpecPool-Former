...
class ConvPool(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride, padding, pool_size):
        ...
        self.relu = nn.ReLU()
        self.pool = nn.MaxPool2d(pool_size)  # 最大池化层
...
class SpecPool(nn.Module):
    def __init__(self, image_size, near_band, num_patches, num_classes, dim, depth, heads, mlp_dim, pool='cls',
        ...
        self.conv_pool = ConvPool(in_channels=1, out_channels=3, kernel_size=(1, 3), stride=1, padding=0,
                                  pool_size=(1, 2))  # 这里添加卷积池化层
    ...
    def forward(self, x, mask=None):
        ...
        x = x.unsqueeze(1)  # 添加一个通道维度，适配Conv2d
        # print(x.shape)
        x = self.conv_pool(x)  # 卷积和池化处理
        # print(x.shape)
        x = x.squeeze(-1)  # 恢复维度 b,c,n
...
if __name__ == '__main__':
    input = torch.randn(64, 200, 5).cuda()
